...
| Relaciones, confianza, romance | `rules_engine.md` 19; `world_bible.md` 12 | `relationshipWithLucas` en `Npc`; `NpcRelationship` para NPC-NPC; `NpcSocialLedger` para caps diarios/antifarmeo; `socialProfileService` pondera personality/values/tolerates/rejects/socialProfile; lecturas en context/npc/search | 12 relaciones NPC-NPC base de Hoshimori vivas; vinculos Lucas-NPC con trust/familiarity/respect/affection/suspicion/fear/jealousy/socialDebt; contexto compacto expone perfil social resumido | partial | `audit:hoshimori-social`; tests de `previewSocialImpact`, perfil NPC y caps diarios en `apiTurnHardening` | Mantener romance fuera hasta que haya canon explicito; ampliar tensiones/secretos solo por eventos |
