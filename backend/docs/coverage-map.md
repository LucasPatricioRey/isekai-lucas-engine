...
- 13 disciplinas/rutas: Mana, Magia, Percepcion magica, Magia ofensiva, Fuego, Rayo/Electricidad, Hielo, Tierra/Viento, Magia defensiva, Resistencia magica, Magia curativa, Magia mental y Magia de invocacion.
- 12 tecnicas/ejercicios/teorias iniciales: respiracion de mana basica, sentir flujo interno, meditacion de mana, percepcion magica basica, teoria de estructura magica principiante, canalizacion segura en reposo, teorias seguras de ofensiva/defensa/curacion/mental/invocacion y acondicionamiento de resistencia magica.
- 10 plantillas bloqueadas de hechizo real (`locked_template`) para probar requisitos sin ensenar hechizos gratis: chispa ofensiva, brasa dirigida, descarga estatica, esquirla de hielo, empuje tierra/viento, resguardo menor, estabilizacion menor, toque de presencia mental, marca de anclaje de invocacion y aguante arcano.
...
| Magia, MP y practica | `rules_engine.md` 14; `world_bible.md` 1.4, 9.3 | MP, skills `skill_mana`/`skill_magia`, `knownSpells`, `MagicDiscipline`, `MagicTechnique`, `CharacterMagicKnowledge`, `magicService`, endpoints `/api/magic/*`, `turn/apply.magicPractice`, `turn/apply.magicPatches` | MP 200/200, `knownSpells: []`, 13 disciplinas, 22 tecnicas/plantillas, 10 plantillas de hechizo real bloqueadas, 0 hechizos avanzados aprendidos por seed | partial | `seed:magic-basics` OK; `audit:magic-basics` OK; previews no mutan; `magicPractice` requiere tiempo/coste y puede consumir MP/EXP sin aprender hechizos; hardening cubre `unlock_skill`, bloqueo por requisitos y practica de chispa conocida; C19 agrega `effectProfile` y bloquea plantillas no ofensivas en `use_magic` | Siguiente: maestro/instructor profundo, resolvers especificos de defensa/curacion/resistencia y reaccion social avanzada |
...
C24A-C24D - Critical Rule Cards:

| Area / seccion | Fuente en docs | Representacion en repo | Representacion en MongoDB vivo | Estado | Evidencia | Proxima accion recomendada |
|---|---|---|---|---|---|---|
| Rule cards criticas | `rules_engine.md` 3.2.1 | `WorldDocumentIndex` con `ruleId`, `domain`, `priority`, `appliesTo`, `must`, `never`, `template`; `seedDocuments.js`; `searchDocs` con filtros | `WorldDocumentIndex` guarda tarjetas `rule_card` junto a chunks de docs | implemented_backend | C24D publica 31 rule cards. `format.skill_progress`, `format.mechanical_changes`, `format.response_hud_exact`, `skills.exp_evaluation_antifarm`, `social.relationship_logic`, `economy.trade_stock_property`, `missions.guild_rewards_registration`, `travel.world_friction_preview` y otras devuelven reglas exactas sin sumar operacion al Action principal. | Mantener sincronizadas con instrucciones GPT compactas y sumar nuevas cards solo cuando cubran reglas criticas que el GPT tiende a compactar mal. |
