...
| Relaciones, confianza, romance | `rules_engine.md` 19; `world_bible.md` 12 | `relationshipWithLucas` en `Npc`; `NpcRelationship` para NPC-NPC; `NpcSocialLedger` para caps diarios/antifarmeo; lecturas en context/npc/search | 12 relaciones NPC-NPC base de Hoshimori vivas; vinculos Lucas-NPC con trust/familiarity/respect/affection/suspicion/fear/jealousy/socialDebt | partial | `audit:hoshimori-social`; tests de `previewSocialImpact` y caps diarios en `apiTurnHardening` | Mantener romance fuera hasta que haya canon explicito; ampliar tensiones/secretos solo por eventos |
