...
- `narrativeHints`/`narrativeContext`/`dramaticContext` y `scene.nearbyNpcs[].dialogueProfile` no suman operaciones: viajan dentro de respuestas existentes para variar escenas, mejorar dialogos, sostener subtexto y mantener HUD final sin tocar calculos mecanicos.
