...
- `applyTurn.evidencePatches` no suma operacion: guarda evidencia/objetos narrativos no vendibles y evita inventar items para muestras fisicas. `worldEventPatches.progress` permite registrar progreso parcial de eventos sin resolverlos.
- `applyTurn.magicPractice` no suma operacion: aplica practica magica real con `timeAdvance` + `activityCost`, consume MP si corresponde y suma EXP validada sin aprender hechizos. `applyTurn.magicPatches` guarda desbloqueos de ramas magicas y conocimiento formal de tecnicas/hechizos.
