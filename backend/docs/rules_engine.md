...
Actualizacion C28: `applyTurn dryRun:true` cubre `magicPractice` y `magicPatches`; `resolveTurn`/`previewResolveTurn` resuelve turnos compuestos normales desde `sequence[]` para viaje, descanso, chequeo, `work_segment`/trabajo parcial, tardanza laboral, disculpa/memoria NPC y EXP fisica conservadora. Si el dominio no esta cubierto por `resolveTurn`, los turnos compuestos deben preferir un solo `applyTurn` con `activitySegments`.
...
- si una accion normal mezcla viaje/descanso/chequeo/trabajo parcial/tardanza/disculpa/memoria NPC, debe preferir `previewResolveTurn`/`resolveTurn` con `sequence[]` antes que armar manualmente rutas, costes, EXP fisica, jobContractPatch y npcMemoryPatches;
- trabajo parcial dentro de turno activo usa `resolveTurn` con `actionFamily:"job_shift"` y `sequence:[{type:"work_segment", minutes, intensity}]`; `job_attendance` solo registra llegada/tardanza/ausencia/presencia formal, y `completeJobShift` solo cierra turno completo con paga/comidas/resumen;
...
Cuando `getCompactContext.commitmentReviewAgenda` o `previewWorldTick.commitmentReviews` exponga un `reviewPlan`, el backend ya hizo la evaluacion offscreen disponible. El narrador no debe inventar otro resultado por comodidad: debe aplicar `suggestedCommitmentPatch`, `suggestedGameStatePatch` u otro patch equivalente y justificado. Si el plan dice `outcome:"fulfill"` para un tramite administrativo, se puede narrar que el NPC/institucion responsable lo resolvio fuera de camara; si dice `reprogram`, debe guardar `blockerSummary` y nuevo `nextCheck`.
...
16. Promesas, estimaciones, tramites, citas, planes importantes y obligaciones futuras usan `commitmentPatches`; si hay `reviewPlan`, usar sus parches sugeridos o equivalente validado; si vencen, requieren revision o no tienen agenda, se cumplen, explican, reprograman o cierran formalmente antes de saltar escenas largas.
