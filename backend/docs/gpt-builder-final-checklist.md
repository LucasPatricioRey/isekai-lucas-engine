...
Cuando cambien `docs/rules_engine.md` o `docs/world_bible.md`, ejecutar `npm run seed:documents` contra MongoDB para que `GET /api/search/docs` vea la misma version que Knowledge. Desde C24C, ese seed tambien publica rule cards criticas consultables por `ruleId`, por ejemplo `format.skill_progress` y `format.mechanical_changes`, sin sumar operaciones al Action principal.
